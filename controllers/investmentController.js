...
const update_investment_type = async (req, res) => {
  try {
    ...
    const updatedAccount = await Investment.findOneAndUpdate(
      filter,
      update,
      options
    );
    if (updatedAccount) {
      ...
      res.status(404).json({
        message: `No account found with InvestmentID ${InvestmentID}`,
      });
    }
  } catch (err) {
...
const delete_investment = async (req, res) => {
  try {
    const InvestmentID = req.body.InvestmentID;
    const deletedAccount = await Investment.findOneAndDelete({
