...
const initialise_transaction = async (req, res) => {
  try {
    ...
    const receiverCustomer = await Customer.findOne(
      receiverCustomerDetailsParams
    );

    const senderAccountCheck = senderAccount ? true : false;
    const receiverAccountCheck = receiverAccount ? true : false;

    const senderNameCheck =
      req.body.sender.CustomerName.toLowerCase() ===
      (senderCustomer ? senderCustomer.Name : "").toLowerCase()
        ? true
        : false;

    const receiverNameCheck =
      req.body.receiver.CustomerName.toLowerCase() ===
      (receiverCustomer ? receiverCustomer.Name : "").toLowerCase()
        ? true
        : false;

    const paymentAmount = req.body.paymentAmount;
    // upper limit check on amount can also be employed
    const paymentAmountCheck =
      paymentAmount > 0 && senderAccount
        ? senderAccount.Balance >= paymentAmount
        : false;

    const proceedPayment =
      paymentAmountCheck &&
      senderAccountCheck &&
      receiverAccountCheck &&
      senderNameCheck &&
      receiverNameCheck;

    // update balance after validation
    if (proceedPayment) {
      Account.updateOne(senderAccountSearchParams, {
        Balance: senderAccount.Balance - paymentAmount,
      })
        .then((resultSender) => {
          Account.updateOne(receiverAccountSearchParams, {
            Balance: receiverAccount.Balance + paymentAmount,
          })
            .then((resultReciver) => {
...
const initialise_investment = async (req, res) => {
  try {
    ...
    const paymentAmountCheck =
      paymentAmount > 0 && account ? account.Balance >= paymentAmount : false;
    const accountCheck = account ? true : false;
    const nameCheck =
      req.body.CustomerName.toLowerCase() ===
    ...
    const proceedPayment =
      paymentAmountCheck && accountCheck && nameCheck && investmentCheck;

    // making changes in database to finalise payment
    if (proceedPayment) {
      Account.updateOne(accountSearchParams, {
        Balance: account.Balance - paymentAmount,
      })
        .then((resultAccount) => {
          Investment.updateOne(investmentSearchParams, {
            Amount: investment.Amount + paymentAmount,
          })
...
const initialise_mutual_fund = async (req, res) => {
  try {
    ...
    // combining all different checks together
    const proceedPayment =
      paymentAmountCheck && accountCheck && nameCheck && mutualFundCheck;
    ...
    if (proceedPayment) {
      Account.updateOne(accountSearchParams, {
        ...
        .then((resultAccount) => {
          MutualFund.updateOne(mutualFundSearchParams, {
            Amount: mutualFund.Amount + paymentAmount,
          })
...
const initialise_insurance = async (req, res) => {
  try {
    ...
    // combining all different checks together
    const proceedPayment =
      paymentAmountCheck && accountCheck && nameCheck && insuranceCheck;
    ...
    if (proceedPayment) {
      Account.updateOne(accountSearchParams, {
        ...
        .then((resultAccount) => {
          Insurance.updateOne(insuranceSearchParams, {
            Amount: insurance.Amount + paymentAmount,
          })
